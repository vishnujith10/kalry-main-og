  ...
  const fetchCardio = async (userId, startISO, endISO) => {
    try {
      ...
      const { data: sessions, error: sErr } = await supabase
        ...
        .order('created_at', { ascending: false });
      if (!sErr && sessions && sessions.length) {
      ...
      // Fallback: cardio_details (no user_id in your schema)
      const { data, error } = await supabase
        .from('cardio_details')
        .select('id, exercise_name, duration, total_rounds, created_at')
        .gte('created_at', startISO)
        .lte('created_at', endISO)
        .order('created_at', { ascending: false });
      if (error) throw error;
      return (data || []).map(cardio => ({
        id: `cardio-${cardio.id}`,
        title: cardio.exercise_name || 'Cardio',
        description: `Rounds: ${cardio.total_rounds ?? 1}   ${formatDuration(cardio.duration || 0)}`,
        type: 'workout',
        _date: cardio.created_at ? cardio.created_at.split('T')[0] : '',
        _time: cardio.created_at ? new Date(cardio.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''
      }));
    } catch (error) {
